    ...
    <table class="w-full">
        ...
        <tbody>
            ...
            <tr class="border-t hover:bg-slate-50 transition-colors" style="border-color:#f1f5f9;">
                ...
                <td class="px-6 py-4 text-center text-sm font-semibold" style="color:#444444;">{{ $row['dispatched'] }}</td>
                <td class="px-6 py-4 text-center text-sm font-semibold" style="color:#222222;">{{ $row['returned']>0?$row['returned']:'—' }}</td>
                <td class="px-6 py-4 text-center text-sm text-gray-500">{{ $row['adjusted']!=0?$row['adjusted']:'—' }}</td>
                <td class="px-6 py-4 text-center text-sm font-bold {{ $row['net_movement']<0?'text-gray-800':'text-gray-700' }}">{{ $row['net_movement'] }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
