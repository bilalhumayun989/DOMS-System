    ...
    <table class="w-full">
        ...
        <tbody>
            ...
            <tr class="border-t hover:bg-slate-50 transition-colors" style="border-color:#f1f5f9;">
                ...
                <td class="px-6 py-4 text-right text-sm font-bold" style="color:#333333;">{{ pkr($row['collections']) }}</td>
                <td class="px-6 py-4 text-right text-sm font-bold {{ $row['shortages']>0?'text-gray-800':'text-gray-300' }}">{{ $row['shortages']>0?pkr($row['shortages']):'—' }}</td>
                <td class="px-6 py-4 text-right text-sm font-semibold" style="color:#444444;">{{ pkr($row['returns']) }}</td>
        ...
        <tfoot>
            <tr style="background:#fafbfc;border-top:2px solid #e2e8f0;">
                ...
                <td class="px-6 py-4 text-right text-sm font-bold" style="color:#333333;">{{ pkr(array_sum(array_column($rows,'collections'))) }}</td>
                <td class="px-6 py-4 text-right text-sm font-bold" style="color:#222222;">{{ pkr(array_sum(array_column($rows,'shortages'))) }}</td>
                <td class="px-6 py-4 text-right text-sm font-bold" style="color:#444444;">{{ pkr(array_sum(array_column($rows,'returns'))) }}</td>
