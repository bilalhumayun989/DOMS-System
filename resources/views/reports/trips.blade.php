...
@section('content')
<div class="mb-5">
    <a href="{{ route('reports.index') }}" class="inline-flex items-center gap-1.5 text-sm font-semibold text-gray-700 hover:text-gray-800">
    ...
    <table class="w-full">
        ...
        <tbody>
            ...
            <tr class="border-t hover:bg-slate-50 transition-colors" style="border-color:#f1f5f9;">
                ...
                <td class="px-6 py-4 text-right text-sm font-bold" style="color:#333333;">{{ pkr($row['collected']) }}</td>
                <td class="px-6 py-4 text-right text-sm font-bold {{ $row['shortage']>0?'text-gray-800':'text-gray-300' }}">{{ $row['shortage']>0?pkr($row['shortage']):'—' }}</td>
            </tr>
            @endforeach
        </tbody>
        <tfoot>
            <tr style="background:#fafbfc;border-top:2px solid #e2e8f0;">
                ...
                <td class="px-6 py-4 text-right text-sm font-bold" style="color:#333333;">{{ pkr(array_sum(array_column($rows,'collected'))) }}</td>
                <td class="px-6 py-4 text-right text-sm font-bold" style="color:#222222;">{{ pkr(array_sum(array_column($rows,'shortage'))) }}</td>
            </tr>
        </tfoot>
    </table>
</div>
