        ...
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr class="border-t hover:bg-slate-50 transition-colors" style="border-color:#f1f5f9;">
                    ...
                    <td class="px-6 py-4"><span class="text-xs font-semibold px-2.5 py-1 rounded-lg bg-gray-100 text-gray-700">{{ $row['action'] }}</span></td>
                    <td class="px-6 py-4 font-mono text-xs font-semibold" style="color:#222222;">{{ $row['entity'] }}</td>
                    <td class="px-6 py-4 text-xs text-gray-600 max-w-xs">{{ $row['details'] }}</td>
                    <td class="px-6 py-4 text-xs font-semibold {{ $row['approved_by']?'text-gray-700':'text-gray-300' }}">{{ $row['approved_by']??'—' }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
