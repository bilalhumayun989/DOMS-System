...
<div class="rounded-2xl p-6 mb-5" style="background: #ffffff; border: 1px solid #e8edf2;">
    <div class="flex items-start justify-between flex-wrap gap-4">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Trip: <a href="{{ route('trips.show',$invoice['trip_id']) }}" class="font-mono text-xs font-bold" style="color:#222222;">{{ $invoice['trip_id_display'] }}</a></p>
        ...
        <div class="px-6 py-4 rounded-2xl text-right" style="background: #eff6ff;">
            ...
            <p class="text-3xl font-bold" style="color:#111111;">{{ pkr($invoice['total_value']) }}</p>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr class="border-t hover:bg-slate-50 transition-colors" style="border-color: #f1f5f9;">
                    ...
                    <td class="px-5 py-3 text-center text-sm font-bold {{ $item['delivered_qty']<$item['ordered_qty']?'text-gray-700':'text-gray-700' }}">{{ $item['delivered_qty'] }}</td>
            ...
            <tfoot>
                <tr style="background:#fafbfc; border-top: 2px solid #e8edf2;">
                    ...
                    <td class="px-5 py-3 text-right text-base font-bold" style="color:#111111;">{{ pkr($invoice['total_value']) }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
    ...
    <div class="rounded-2xl overflow-hidden" style="background: #ffffff; border: 1px solid #e8edf2;">
        <div class="px-6 py-4 flex items-center gap-2" style="border-bottom: 1px solid #f1f5f9;">
            <div class="w-1 h-5 rounded-full bg-gray-600"></div>
        ...
        <div class="divide-y" style="divide-color: #f1f5f9;">
            ...
            <div class="px-6 py-4">
                <div class="flex justify-between items-center mb-1">
                    <span class="text-lg font-bold" style="color:#333333;">{{ pkr($col['amount']) }}</span>
                    <span class="text-xs font-semibold px-2 py-1 rounded-lg {{ $col['method']==='Cash'?'bg-gray-100 text-gray-700':($col['method']==='Cheque'?'bg-gray-100 text-gray-700':'bg-gray-100 text-gray-700') }}">{{ $col['method'] }}</span>
                </div>
