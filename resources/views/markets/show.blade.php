...
<div class="rounded-2xl p-6 mb-5" style="background: #ffffff; border: 1px solid #e8edf2;">
    <div class="flex items-start justify-between flex-wrap gap-4 mb-5">
        ...
        <div class="px-5 py-3 rounded-2xl text-right" style="background: #f0f0f0;">
            <p class="text-xs font-semibold uppercase tracking-wide mb-1" style="color: #999999;">Outstanding</p>
            <p class="text-2xl font-bold" style="color: #222222;">{{ pkr($market['outstanding_balance']) }}</p>
        </div>
    </div>
    <div class="grid grid-cols-3 gap-4 pt-5" style="border-top: 1px solid #f1f5f9;">
        @foreach([['Total Invoices',$market['total_invoices'],false,'#333333','#f0f0f0'],['Total Value',$market['total_value'],true,'#222222','#efefef'],['Collected',$market['total_collected'],true,'#333333','#f0f0f0']] as [$l,$v,$f,$c,$b])
        ...
        <table class="w-full">
            ...
            <tbody>
                ...
                <tr class="border-t hover:bg-slate-50 transition-colors" style="border-color: #f1f5f9;">
                    <td class="px-5 py-3"><a href="{{ route('invoices.show',$inv['id']) }}" class="font-mono text-xs font-bold" style="color:#222222;">{{ $inv['invoice_number'] }}</a></td>
                    <td class="px-5 py-3 text-xs text-gray-500">{{ $inv['date'] }}</td>
                    <td class="px-5 py-3"><a href="{{ route('trips.show',$inv['trip_db_id']) }}" class="font-mono text-xs font-semibold" style="color:#222222;">{{ $inv['trip_id'] }}</a></td>
