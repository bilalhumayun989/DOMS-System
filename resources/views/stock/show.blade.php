...
<div class="rounded-2xl p-6 mb-5" style="background: #ffffff; border: 1px solid #e8edf2;">
    <div class="flex items-start justify-between flex-wrap gap-4">
        ...
        <div class="flex gap-4">
            <div class="px-6 py-4 rounded-2xl text-center" style="background: #f0f0f0;">
                <p class="text-xs font-semibold uppercase tracking-wide mb-1" style="color: #999999;">Current Stock</p>
                <p class="text-3xl font-bold" style="color: #222222;">{{ $sku['current_stock'] }}</p>
            ...
            <tr class="border-t hover:bg-slate-50 transition-colors" style="border-color: #f1f5f9;">
                ...
                <td class="px-6 py-4">
                    <span class="text-xs font-semibold px-2.5 py-1 rounded-lg
                        {{ $mov['type']==='Dispatch'?'bg-gray-100 text-gray-700':($mov['type']==='Return'?'bg-gray-100 text-gray-700':'bg-gray-100 text-gray-600') }}">
                        {{ $mov['type'] }}
                    </span>
                </td>
                <td class="px-6 py-4">
                    @if($mov['trip_db_id'])
                    <a href="{{ route('trips.show',$mov['trip_db_id']) }}" class="font-mono text-xs font-bold" style="color:#222222;">{{ $mov['trip_id'] }}</a>
                    @else<span class="text-gray-300 text-xs">—</span>@endif
                </td>
                <td class="px-6 py-4 text-center text-sm font-bold {{ $mov['qty_change']<0?'text-gray-800':'text-gray-700' }}">
