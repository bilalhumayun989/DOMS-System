...
<div x-data="{ open: false, mode: 'create', selected: null }">
    <div class="page-card">
        <div class="page-card-header flex items-center justify-between">
            <div>
                <div class="flex items-center gap-2">
                    <h2 class="page-card-title">Settlements</h2>
                ...
                <p class="page-card-sub">{{ count($settlements) }} settlements recorded</p>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full doms-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="font-mono text-xs font-semibold text-slate-800">{{ $s['ref'] }}</td>
                        <td class="font-semibold text-slate-800">{{ $s['party'] }}</td>
                        <td class="text-slate-600 text-xs">{{ $s['payment_method'] }}</td>
                        <td class="text-slate-600 text-xs">{{ $s['date'] }}</td>
                        <td class="text-right font-bold text-slate-900">{{ pkr($s['amount']) }}</td>
                        <td class="text-center">
                            <div class="flex items-center justify-center gap-1.5">
                                <button @click="mode = 'view'; selected = {{ json_encode($s) }}; open = true;" class="btn-row btn-view">View</button>
                                <button @click="mode = 'edit'; selected = {{ json_encode($s) }}; open = true;" class="btn-row btn-edit">Edit</button>
                                <button @click="mode = 'delete'; selected = {{ json_encode($s) }}; open = true;" class="btn-row btn-delete">Delete</button>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="6" class="text-center py-6 text-slate-400 text-xs">No settlements recorded yet.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div x-show="open" class="modal-panel" style="max-width: 28rem; width: 100%; padding: 1.5rem;" x-cloak>
        <template x-if="mode === 'create' || mode === 'edit'">
            <div>
                <h3 class="text-base font-bold text-slate-900 mb-4" x-text="mode === 'create' ? 'Add Demo Settlement' : 'Edit Demo Settlement'"></h3>
                <form :action="mode === 'create' ? '{{ route('demo.settlements.store') }}' : '/demo/settlements/' + selected?.id" method="POST" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <input type="text" name="party" required class="modal-input" :value="selected?.party ?? ''" placeholder="e.g. Distributor Settlement">
                    </div>
                    <div>
                        <label class="modal-label">Payment Method</label>
                        <select name="payment_method" class="modal-input">
                    ...
                    <div>
                        <label class="modal-label">Settlement Amount (PKR)</label>
                        <input type="number" step="0.01" name="amount" required class="modal-input" :value="selected?.amount ?? ''" placeholder="50000">
                    </div>
                    <div class="flex items-center justify-end gap-3 pt-2">
                        <button type="button" @click="open = false" class="btn-modal-cancel">Cancel</button>
                        <button type="submit" class="btn-modal-save" x-text="mode === 'create' ? 'Record Settlement' : 'Save Changes'"></button>
                    </div>
                </form>
            </div>
        </template>
        ...
        <template x-if="mode === 'view'">
            <div>
                <h3 class="text-base font-bold text-slate-900 mb-4">Settlement Details</h3>
                <div class="space-y-3 text-xs">
                    ...
                    <div class="flex justify-between py-1 border-b border-slate-100"><span class="text-slate-500 font-semibold">Payment Method:</span><span class="font-bold text-slate-800" x-text="selected?.payment_method"></span></div>
        ...
        <template x-if="mode === 'delete'">
            <div>
                <h3 class="text-base font-bold text-slate-900 mb-2">Delete Settlement</h3>
                <p class="text-xs text-slate-600 mb-4">Are you sure you want to delete settlement <span class="font-bold text-slate-900" x-text="selected?.ref"></span>?</p>
                <form :action="'/demo/settlements/' + selected?.id" method="POST">
                    @csrf
