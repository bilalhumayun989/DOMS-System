...
<style>
    .doms-kpi-grid   { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 1.25rem; }
    .doms-middle-grid{ display: grid; grid-template-columns: minmax(0, 2.1fr) minmax(0, 1fr); gap: 1.5rem; }
    .doms-charts-grid{ display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 1.5rem; }
    ...
    @media (max-width: 640px) { .doms-kpi-grid { grid-template-columns: repeat(1, minmax(0, 1fr)); } }
</style>
...
$iconBgMap    = ['blue'=>'bg-gray-100 text-gray-700 border-gray-300','green'=>'bg-gray-100 text-gray-700 border-gray-300','red'=>'bg-gray-100 text-gray-700 border-gray-300','amber'=>'bg-gray-100 text-gray-700 border-gray-300'];
$borderLeftMap= ['blue'=>'border-l-4 border-l-gray-500','green'=>'border-l-4 border-l-gray-500','red'=>'border-l-4 border-l-gray-500','amber'=>'border-l-4 border-l-gray-500'];
@endphp
...
<div class="space-y-6">
    ...
    {{-- TODAY'S TRIPS + QUICK DAY SELECTOR --}}
    <div class="doms-middle-grid">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden min-w-0">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                ...
                <a href="{{ route('demo.trips') }}" class="text-xs font-bold text-gray-700 hover:text-gray-900 transition-colors">View All →</a>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100 font-semibold text-slate-700">
                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            <td class="px-5 py-3.5 font-mono font-bold text-gray-800">
                                <a href="{{ route('demo.trips.show', $trip['id']) }}" class="hover:underline">{{ $trip['route_id'] }}</a>
                            </td>
                            <td class="px-5 py-3.5 font-bold text-slate-900">{{ $trip['deliveryman'] }}</td>
                            ...
                            <td class="px-5 py-3.5 text-center">
                                <a href="{{ route('demo.trips.show', $trip['id']) }}"
                                   class="text-xs font-bold px-3 py-1.5 rounded-xl bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors inline-block">View →</a>
                            </td>
                        </tr>
                        @endforeach
                        @if(count($todaysTrips) === 0)
                        <tr>
                            <td colspan="6" class="px-5 py-10 text-center text-slate-400 font-medium">
                                No trips yet in this demo session.
                                <a href="{{ route('demo.trips') }}" class="text-gray-700 font-semibold underline ml-1">Create a trip →</a>
                            </td>
                        </tr>
                        @endif
                    </tbody>
    ...
    {{-- SHORTAGES (EMPTY IN DEMO) --}}
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div class="flex items-center gap-2">
                <h3 class="font-black text-slate-900 text-base">Daily Shortages &amp; Recoveries</h3>
                <span class="text-xs font-bold px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">0</span>
            </div>
            <a href="{{ route('demo.settlements') }}" class="text-xs font-bold text-gray-700 hover:text-gray-900 transition-colors">View All →</a>
        </div>
        <div class="px-6 py-10 text-center">
            <p class="text-slate-400 text-sm font-medium">No shortage records in demo session. Close a trip to generate shortage data.</p>
        </div>
    </div>
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <span class="text-2xl font-black text-slate-900 mt-2 block tracking-tight">{{ count($todaysTrips) }} Demo Trips</span>
                </div>
                <span class="text-xs font-bold text-gray-700 bg-gray-100 px-2.5 py-1 rounded-full border border-gray-300">Demo Data</span>
            </div>
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <span class="text-2xl font-black text-slate-900 mt-2 block tracking-tight">PKR 48,000.00</span>
                </div>
                <span class="text-xs font-bold text-gray-700 bg-gray-100 px-2.5 py-1 rounded-full">Demo Summary</span>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    new ApexCharts(document.querySelector("#trip-volume-chart"), {
        series: [{ name: 'Trips', data: [0, 0, 1, 0, 0, 0, 0] }],
        ...
        xaxis: { categories: ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'], axisBorder: { show: false }, axisTicks: { show: false }, labels: { style: { colors: '#94A3B8', fontSize: '12px', fontWeight: 600 } } },
        yaxis: { labels: { style: { colors: '#CBD5E1', fontSize: '11px', fontWeight: 500 } } },
        grid: { borderColor: '#F1F5F9', strokeDashArray: 4 },
        tooltip: { theme: 'dark', y: { formatter: (v) => v + " trips" } }
    }).render();

    new ApexCharts(document.querySelector("#collections-trend-chart"), {
        series: [{ name: 'Collection (PKR)', data: [0, 0, 48000, 0, 0, 0, 0] }],
        ...
        xaxis: { categories: ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'], axisBorder: { show: false }, axisTicks: { show: false }, labels: { style: { colors: '#94A3B8', fontSize: '12px', fontWeight: 600 } } },
        yaxis: { labels: { style: { colors: '#CBD5E1', fontSize: '11px', fontWeight: 500 }, formatter: (v) => "Rs " + (v/1000) + "k" } },
        grid: { borderColor: '#F1F5F9', strokeDashArray: 4 },
        tooltip: { theme: 'dark', y: { formatter: (v) => "PKR " + v.toLocaleString() } }
    }).render();
});
</script>
@endsection
